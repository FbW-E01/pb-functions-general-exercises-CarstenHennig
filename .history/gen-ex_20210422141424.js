...
// 2. Write a function that takes in one parameter and returns that parameter repeated five times.
// For the input "Cat" it would return "CatCatCatCatCat"

let words = "";
function repeating(word) {
  for (let i = 0; i < 5; i++) {
    words = words + word;
  }
  console.log(words);
...
// 3. Write a function that takes in two parameters, the first one should be a number
// and the 2nd one should be a string. The functions returns the 2nd argument repeated
// as many times as the 1st argument defins. For the inputs `3` and "Woah" it should return `WoahWoahWoah`

let sentence = "";
function doTheSame(num, saySomething) {
  for (var i = 0; i < num; i++) {
    sentence = sentence + saySomething;
  }
  console.log(sentence);
}

doTheSame(5, "Superkalifragilistischexpialigetisch");
...
let numArr = [1, 6, 83, 91, 0, -4, 1337, 5];
...
function printIfDivisibleByTen(val) {
  if (val / 10) {
    console.log("Even 10!");
  }
}
...
//     Part b) write a for-loop that runs 125 times, each time calling the function you created
//in part a with the current iteration number.
...
stringOrNotString = (givenString1, givenString2) => {
  if (typeof givenString1 === "string") {
    if (typeof givenString2 === "string") {
...
function printFirstWord(whatYouSay) {
  whatYouSay = whatYouSay.split(" ");
  console.log(whatYouSay);
  console.log(whatYouSay[0]);
}

printFirstWord("Hello You");

// 10. *HARD* Write an arrow function that takes in one string.
// The function should return a new string that where each word in the input string is repeated
// by the amount of words in the input string. Examples:
...
const wordRepeater = (firstString) => {
  let wordts = firstString.split(" ");
  let result = [];
  for (var i = 0; i < wordts.length; i++) {
    let twice = wordts[i];
    for (var j = 0; j < wordts.length; j++) {
      result.push(twice);
    }
  }
  return result.join(" ");
...
const firstLetters = (newStrings) => {
  newStrings = newStrings.split(" ");
  for (var i = 0; i < newStrings.length; i++) {
    let selString = newStrings[i];
    console.log(selString[0]);
  }
};
...
// You might need to search Google to find out how can you identify if the parameter is an array -
// but there are other ways to solve this too!

const lengthOfStringArray = (myInput) =>
  typeof myInput === "string" || "array" ? myInput.length : null;

console.log(lengthOfStringArray("Hello, hello!"));
console.log(lengthOfStringArray([1, 2, 3, 4, 5]));
console.log(lengthOfStringArray(["a", "b", "c", "d"]));
console.log(lengthOfStringArray(112));
...
const explode = (x) => (typeof x.split === "string" ? x.split("") : null);
